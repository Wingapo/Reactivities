...
const locationIQApi = ` https://api.locationiq.com/v1/autocomplete?key=${import.meta.env.VITE_LOCATION_IQ_KEY}`
...
const LocationInput = <T extends FieldValues>(props: Props<T>) => {
  ...
  useEffect(() => {
    if (field.value && typeof field.value === 'object') {
      setInputValue(field.value.venue as string || '');
    } else {
  ...
  const fetchSuggestions = useMemo(
    () => debounce(async (query: string) => {
      ...
      try {
        const response = await axios.get<LocationIQSuggestion[]>(`${locationIQApi}&q=${query}`);
